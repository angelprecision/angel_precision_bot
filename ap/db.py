...
# FIX: run_with_retry now catches "disk I/O error" (Render ephemeral disk race condition)
...
# ============================================================
def run_with_retry(fn: Callable[[], Any], retries: int = 30, base_sleep: float = 0.1, max_sleep: float = 2.0):
    """
    Retry SQLite operations on transient errors.
    FIX: catches 'disk I/O error' in addition to lock errors.
    Render free tier throws disk I/O errors when multiple threads
    hit SQLite simultaneously — not a real disk failure, just contention.
    """
    delay = base_sleep
    last_err = None
    ...
    for _ in range(retries):
        try:
            ...
            last_err = e
            msg = str(e).lower()
            # FIX: added disk i/o, unable to open, i/o error
            if (
...
def init_db():
    with conn() as c:
        ...
        """)
        try: ex("ALTER TABLE audit_log ADD COLUMN client_id TEXT;")
        except: pass
        try: ex("CREATE INDEX IF NOT EXISTS idx_audit_log_client_id ON audit_log(client_id);")
        except: pass
        ...
        """)
        try: ex("ALTER TABLE positions ADD COLUMN client_id TEXT NOT NULL DEFAULT 'default';")
        except: pass
        try:
        ...
        try: ex("CREATE INDEX IF NOT EXISTS idx_client_state_day_key ON client_state(day_key);")
        except: pass
...
def update_order(
    ...
    *,
    ...
    if broker_order_id is not None:
        updates.append("broker_order_id=?"); params.append(broker_order_id)
    if last_error is not None:
        updates.append("last_error=?"); params.append(last_error)
    if filled_qty is not None:
        updates.append("filled_qty=?"); params.append(int(filled_qty))
    updates.append("updated_ts=?"); params.append(now_utc_iso())
...
def update_client_state(client_id: str = "default", updates: dict | None = None):
    ...
    with conn() as c:
        existing = run_with_retry(lambda: c.execute(
            "SELECT 1 FROM client_state WHERE client_id=?", (client_id,)
        ).fetchone())
        if not existing:
            run_with_retry(lambda: c.execute(
                """INSERT INTO client_state (
                    ...
                    kill_switch, mode, last_heartbeat_ts, day_key
    ...
    allowed = set()
    for r in cols:
        try: allowed.add(r["name"])
        except: allowed.add(r[1])
    ...
    with conn() as c:
        run_with_retry(lambda: c.execute(
            f"UPDATE client_state SET {set_clause} WHERE client_id=?", values
        ))
...
def create_client(
    ...
    max_concurrent_positions: int = 3, daily_max_loss_pct: float = 0.05,
...
def upsert_client(client_id: str, **kwargs):
    with conn() as c:
        existing = c.execute("SELECT client_id FROM clients WHERE client_id=?", (client_id,)).fetchone()
        now = now_utc_iso()
        if existing:
            updates = []; params = []
            ...
            run_with_retry(lambda: c.execute(
                f"UPDATE clients SET {', '.join(updates)} WHERE client_id=?", params
            ))
        else:
            ...
            run_with_retry(lambda: c.execute(
                f"INSERT INTO clients ({cols}) VALUES ({placeholders})", list(kwargs.values())
            ))
...
def get_all_positions(client_id: str | None = None) -> list[dict]:
    with conn() as c:
        if client_id:
            rows = run_with_retry(lambda: c.execute(
                "SELECT * FROM positions WHERE client_id=? ORDER BY entry_ts DESC", (client_id,)
            ).fetchall())
        else:
            rows = run_with_retry(lambda: c.execute(
...
def get_orders_for_client(client_id: str) -> list[dict]:
    with conn() as c:
        rows = run_with_retry(lambda: c.execute(
            "SELECT * FROM orders WHERE client_id=? ORDER BY created_ts DESC", (client_id,)
        ).fetchall())
        return [dict(r) for r in rows]
...
def get_all_orders() -> list[dict]:
    with conn() as c:
        rows = run_with_retry(lambda: c.execute(
            "SELECT * FROM orders ORDER BY created_ts DESC"
        ).fetchall())
        return [dict(r) for r in rows]
...
def get_audit_logs(client_id: str | None = None, limit: int = 100) -> list[dict]:
    with conn() as c:
        if client_id:
            rows = run_with_retry(lambda: c.execute(
                "SELECT * FROM audit_log WHERE client_id=? ORDER BY ts DESC LIMIT ?", (client_id, limit)
            ).fetchall())
        else:
            rows = run_with_retry(lambda: c.execute(
...
def get_order_by_id(local_order_id: str) -> dict | None:
    with conn() as c:
        row = run_with_retry(lambda: c.execute(
            "SELECT * FROM orders WHERE local_order_id=?", (local_order_id,)
        ).fetchone())
        return dict(row) if row else None
...
def delete_client(client_id: str):
    with conn() as c:
        for table in ["audit_log", "trade_queue", "orders", "positions", "client_state", "clients"]:
            run_with_retry(lambda t=table: c.execute(f"DELETE FROM {t} WHERE client_id=?", (client_id,)))
...
def list_orders(client_id: str | None = None, limit: int = 200, status: str | None = None) -> list[dict]:
    with conn() as c:
        if client_id and status:
            ...
            rows = run_with_retry(lambda: c.execute(
                "SELECT * FROM orders ORDER BY created_ts DESC LIMIT ?", (limit,)
            ).fetchall())
        return [dict(r) for r in rows]
...
def list_positions(client_id: str | None = None, limit: int = 200, status: str | None = None) -> list[dict]:
    with conn() as c:
        if client_id and status:
            ...
            rows = run_with_retry(lambda: c.execute(
                "SELECT * FROM positions ORDER BY entry_ts DESC LIMIT ?", (limit,)
            ).fetchall())
        return [dict(r) for r in rows]
