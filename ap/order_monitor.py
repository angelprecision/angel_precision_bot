...
# ── PR #404: Pending-entry ownership classifiers (pure, stateless) ───────────

from typing import NamedTuple as _NT_om  # local alias — typing already imported above

PENDING_OWNER_ACTIVE       = "PENDING_OWNER_ACTIVE"
PENDING_OWNER_STALE        = "PENDING_OWNER_STALE"
PENDING_OWNER_TERMINAL     = "PENDING_OWNER_TERMINAL"
PENDING_OWNER_MISSING      = "PENDING_OWNER_MISSING"
PENDING_OWNER_CROSS_CLIENT = "PENDING_OWNER_CROSS_CLIENT"
PENDING_OWNER_CROSS_MODE   = "PENDING_OWNER_CROSS_MODE"
PENDING_OWNER_CONFLICT     = "PENDING_OWNER_CONFLICT"
PENDING_OWNER_DB_ERROR     = "PENDING_OWNER_DB_ERROR"

_TERMINAL_PE_STATUSES = frozenset({
    "CANCELED", "CANCELLED", "EXPIRED", "REJECTED",
    "ERROR", "FAILED", "TERMINAL", "CLOSED", "FILLED",
})
_BROKER_SUBMIT_PE_STATUSES = frozenset({
    "SUBMITTED", "ACKNOWLEDGED", "PARTIAL_FILL", "PARTIALLY_FILLED",
    "ACCEPTED", "OPEN",
})
_ACTIVE_PE_STATUSES = frozenset({
    "CREATED", "PENDING_TRIGGER",
}) | _BROKER_SUBMIT_PE_STATUSES

_ORPHAN_AGE_MINUTES = 20   # CREATED/PENDING_TRIGGER with no ownership proof
...
def _classify_pending_entry_ownership(
    existing: dict,
    *,
    watcher_owned: bool,
    recovery_owned: bool,
    broker_terminal: bool,
) -> _PendingOwnershipResult:
    """Classify one pending-entry row.  Pure/stateless; no DB access."""
    if not isinstance(existing, dict):
        return _PendingOwnershipResult(PENDING_OWNER_MISSING, False, False, "not_a_dict")
    ...
    if not status:
        return _PendingOwnershipResult(PENDING_OWNER_MISSING, False, True, "missing_status")
    ...
    if status in _ACTIVE_PE_STATUSES:
        ...
        broker_oid  = str(existing.get("broker_order_id") or "").strip()
        ...
        if created_raw:
            try:
                ...
                _ts = _dt.fromisoformat(str(created_raw).replace("Z", "+00:00"))
...
def _pending_entry_blocks_candidate(
    existing: dict,
    candidate: dict,
    *,
    watcher_owned: bool,
