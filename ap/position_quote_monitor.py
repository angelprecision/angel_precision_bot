...
def _broker_transport_identity(broker) -> str:
    ...
    return str(base_url).strip().rstrip("/").lower() or type(broker).__name__
...
class APPositionQuoteMonitor:
    def __init__(
        ...
        self._health_key = (
            f"ap_quote_monitor:{self.client_id}:"
            f"{self.execution_mode or 'unknown'}"
        )
        self._cache_namespace = (
            f"{self.execution_mode or 'unknown'}|"
            f"{_broker_transport_identity(self.broker)}"
        )

        self._last_successful_refresh_ts = 0.0
        self._last_active_position_count = 0
        self._last_direct_recovery_ts: dict[str, float] = {}
        self._coverage_degraded = False
    ...
    def stop(self):
        ...
        try:
            ...
            HEALTH.stopped(self._health_key, reason="quote_monitor_stopped")
        except Exception:
            pass
    ...
    def last_successful_refresh_age_sec(self) -> float:
        ts = getattr(self, "_last_successful_refresh_ts", 0.0)
        if ts <= 0:
            return 9e9
        return time.time() - ts

    def _cache_key(self, symbol: str) -> str:
        ns = getattr(self, "_cache_namespace", "unknown|unknown")
        return f"{ns}|{str(symbol or '').upper().strip()}"

    def is_healthy(self) -> bool:
        if not self.is_alive():
            return False

        if self.last_cycle_age_sec() > HEARTBEAT_DEGRADED_SEC:
            return False

        if getattr(self, "_last_active_position_count", 0) <= 0:
            return True

        return (
            self.last_successful_refresh_age_sec()
    ...
    def metrics_snapshot(self) -> dict:
        ...
        m["health_key"] = getattr(self, "_health_key", "ap_quote_monitor:unknown:unknown")
        m["execution_mode"] = getattr(self, "execution_mode", "")
        m["account_id"] = getattr(self, "account_id", "")
        m["active_position_count"] = getattr(self, "_last_active_position_count", 0)
        m["last_successful_refresh_age_sec"] = round(
            self.last_successful_refresh_age_sec(), 2
        )
        return m
    ...
    def _loop(self):
        ...
        try:
            ...
            _QPM_HEALTH.ensure_registered(
                self._health_key, _QPM_CRIT.CRITICAL, stale_after_s=30.0
            )
        ...
        while not self._stop.is_set():
            ...
            try:
                ...
                cycle = self._refresh_once() or {}
                active_positions = int(cycle.get("active_positions") or 0)
                refreshed_positions = int(
                    cycle.get("refreshed_positions") or 0
                )
                coverage_complete = bool(
                    cycle.get("coverage_complete")
                )

                self._last_active_position_count = active_positions
                self._consecutive_failures = 0
                ...
                if active_positions == 0:
                    ...
                    self._last_successful_refresh_ts = time.time()
                    self._coverage_degraded = False
                    if _qpm_health_available:
                        try:
                            from ap_health_registry import HEALTH as _QPM_HEALTH
                    ...
                    if not self._coverage_degraded:
                        self._coverage_degraded = True
                        if _qpm_health_available:
                            try:
                                from ap_health_registry import HEALTH as _QPM_HEALTH
                                _QPM_HEALTH.report_error(
                                    self._health_key,
                                    (
                                        "position_quote_coverage_incomplete:"
                                        f"{refreshed_positions}/{active_positions}"
                                    ),
                                    fatal=False,
                                    metrics={
                                        "active_positions": active_positions,
    ...
    def _refresh_once(self):
        ...
        self._last_active_position_count = len(positions)
        ...
        with lock_ctx:
            for pos in positions:
                ...
                uq, oq, _direct_recovered = self._maybe_direct_recover_position(
                    position_id=pid,
                    underlying=t,
                    contract=c,
                    pos=pos,
                ...
                option_truth_refreshed = (
                    bid > 0
                    or _safe_float(oq.get("last"), 0.0) > 0
                    or _safe_float(oq.get("mark"), 0.0) > 0
                )
                if option_truth_refreshed:
        ...
        return {
            ...
            "refreshed_positions": refreshed_positions,
            "coverage_complete": (
                refreshed_positions == len(positions)
            ),
        }
    ...
    # ── Direct uncached recovery helpers ────────────────────────────────────
    def _direct_recovery_due(self, position_id: str) -> bool:
        now = time.time()

        global _SHARED_BACKOFF_UNTIL
        if now < _SHARED_BACKOFF_UNTIL:
            self._metrics["direct_recovery_suppressed"] += 1
            return False

        recovery_ts = getattr(self, "_last_direct_recovery_ts", None)
        if recovery_ts is None:
            self._last_direct_recovery_ts = {}
            recovery_ts = self._last_direct_recovery_ts

        key = str(position_id or "").strip()
        last = recovery_ts.get(key, 0.0)
        if now - last < QPM_DIRECT_RECOVERY_MIN_INTERVAL_SEC:
            self._metrics["direct_recovery_suppressed"] += 1
            return False
    ...
    def _option_quote_has_truth(self, quote: dict) -> bool:
        if not isinstance(quote, dict):
            return False
        return any(
            _safe_float(quote.get(key), 0.0) > 0
            for key in ("bid", "last", "mark")
        )
    ...
    def _maybe_direct_recover_position(
        ...
        *,
        ...
        current_option_ok = self._option_quote_has_truth(option_quote)
        current_underlying_ok = (
            self._extract_underlying_price(underlying_quote) > 0
        )

        if current_option_ok and current_underlying_ok:
            return underlying_quote, option_quote, False
        ...
        opt_age = (
            (now_utc - prior_opt_ts).total_seconds()
            if prior_opt_ts else 999.0
        )
        und_age = (
            (now_utc - prior_und_ts).total_seconds()
            if prior_und_ts else 999.0
        )

        if (
            opt_age < DEGRADED_MAX_SEC
            and und_age < DEGRADED_MAX_SEC
        ):
            return underlying_quote, option_quote, False

        if not self._direct_recovery_due(position_id):
            return underlying_quote, option_quote, False

        symbols = [
            symbol
            for symbol in {
                str(underlying or "").upper().strip(),
                str(contract or "").upper().strip(),
            }
            if symbol
        ]

        self._metrics["direct_recovery_attempts"] += 1
        recovered = self._fetch_batch(symbols)

        receipt_epoch = time.time()
        normalized = {}
        for symbol, quote in (recovered or {}).items():
            payload = dict(quote or {})
            payload["_ap_receipt_epoch"] = receipt_epoch
            normalized[str(symbol).upper()] = payload

        recovered_underlying = (
            normalized.get(str(underlying or "").upper())
            or underlying_quote
        )
        recovered_option = (
            normalized.get(str(contract or "").upper())
            or option_quote
        )

        success = (
            self._option_quote_has_truth(recovered_option)
            and self._extract_underlying_price(recovered_underlying) > 0
        )

        if success:
            self._metrics["direct_recovery_successes"] += 1
            log.warning(
                "[%s] QPM_DIRECT_RECOVERY_SUCCEEDED "
                "position=%s contract=%s underlying=%s",
                self.client_id,
                position_id,
                contract,
                underlying,
            ...
            self._metrics["direct_recovery_failures"] += 1
            log.critical(
                "[%s] QPM_DIRECT_RECOVERY_FAILED "
                "position=%s contract=%s underlying=%s "
                "option_age=%.1fs underlying_age=%.1fs",
                self.client_id,
                position_id,
                contract,
                underlying,
        ...
        return recovered_underlying, recovered_option, success
        ...
        if fresh:
            with _SHARED_CACHE_LOCK:
                ...
                for s, q in fresh.items():
                    _SHARED_CACHE[self._cache_key(s)] = {"quote": q, "ts": ts}
                    out[s] = q
            self._rate_limit_backoff_sec = RATE_LIMIT_BACKOFF_BASE_SEC
        return out
