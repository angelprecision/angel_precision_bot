...
The legacy top-level ``ap_exit_engine.py`` remains the only reducer, broker
submission authority, order owner, fill owner, and reconciliation owner. This
package shadows its import surface and tightens quote truth only for positions
owned by the production APExitEngine below.
"""
...
ExitDecision = _base.ExitDecision
ExecutableQuoteApplication = _base.ExecutableQuoteApplication
...
_base_is_option_quote_stale = _base._is_option_quote_stale

# Preserve the public class identity expected by existing runtime guards and
# tests. The class implementation remains the exact legacy class.
ExitDecision.__module__ = __name__
ExecutableQuoteApplication.__module__ = __name__
...
TOUCHED_PROFIT_CONFIRM_MAX_GAP_SEC = float(
    _os.getenv("TOUCHED_PROFIT_CONFIRM_MAX_GAP_SEC", "15")
)
EXIT_DECISION_QUOTE_MAX_AGE_SEC = float(
    _os.getenv("EXIT_DECISION_QUOTE_MAX_AGE_SEC", "12")
)
EXIT_DECISION_MAX_FUTURE_SKEW_SEC = float(
    _os.getenv("EXIT_DECISION_MAX_FUTURE_SKEW_SEC", "2")
)
EXIT_DECISION_MAX_CYCLE_SKEW_SEC = float(
    _os.getenv("EXIT_DECISION_MAX_CYCLE_SKEW_SEC", "1")
)
SOFT_EXIT_THESIS_GRACE_SEC = float(
    _os.getenv("SOFT_EXIT_THESIS_GRACE_SEC", "300")
)
SOFT_EXIT_META_PERSIST_SEC = float(
    _os.getenv("SOFT_EXIT_META_PERSIST_SEC", "5")
)
...
class ManagedPosition(_BaseManagedPosition):
    ...
    def __setattr__(self, name: str, value: _Any) -> None:
        enabled = bool(
            self.__dict__.get("executable_bid_soft_exit_truth_enabled", False)
        )
        ...
        if name in {
            ...
            canonical = float(
                self.__dict__.get("peak_executable_pnl_pct", 0.0) or 0.0
            )
            try:
                value = min(
                    max(float(value or 0.0), 0.0), max(canonical, 0.0)
                )
            except Exception:
    ...
    def is_at_target(self) -> bool:
        if self.executable_bid_soft_exit_truth_enabled:
            if not _validate_exit_truth_snapshot(self).valid:
                return False
        return bool(_BaseManagedPosition.is_at_target.fget(self))
    ...
    def is_at_stop(self) -> bool:
        if self.executable_bid_soft_exit_truth_enabled:
            if not _validate_exit_truth_snapshot(self).valid:
                return False
        return bool(_BaseManagedPosition.is_at_stop.fget(self))


ManagedPosition.__module__ = __name__


def _enabled(pos: _Any) -> bool:
    return (
        getattr(pos, "executable_bid_soft_exit_truth_enabled", False) is True
    )
...
def _apply_option_quote_for_decision(
    ...
    *,
    ...
    if not _enabled(pos):
        return _base_apply_option_quote(
            pos,
            bid=bid,
            ask=ask,
            mark=mark,
    ...
    if valid:
        src = source or (
            "executable_bid"
            if pricing_mode in {"paper", "live"}
            else "executable_bid_live_risk_unproven"
        )
    else:
        src = (
            "bid_missing"
    ...
    for name, value in (
        ("analytics_mark_price", analytics),
        ("analyticsmarkprice", analytics),
        ("current_bid", bid),
        ("currentbid", bid),
        ("current_ask", ask),
        ("currentask", ask),
        ("current_option_price", executable),
        ("currentoptionprice", executable),
        ("executable_exit_price", executable),
        ("executable_quote_valid", valid),
        ("live_executable_price_source", src),
        ("liveexecutablepricesource", src),
        ("pricing_mode", pricing_mode),
        ("raw_execution_mode", raw_mode),
    ):
        try:
    ...
    return ExecutableQuoteApplication(
        pricing_mode, executable, valid, analytics, src
    )
...
def _validate_exit_truth_snapshot(
    pos: ManagedPosition,
    *,
    now_utc: _Optional[_datetime] = None,
) -> ExitTruthStatus:
    ...
    for key, wanted in expected.items():
        if not wanted:
            return ExitTruthStatus(
                False,
                f"position_identity_unproven:{key}",
                {"expected": expected, "actual": actual},
            )
        if actual.get(key) != wanted:
            return ExitTruthStatus(
                False,
                f"snapshot_identity_mismatch:{key}",
                {"expected": expected, "actual": actual},
            )
    ...
    if underlying_ts is None:
        return ExitTruthStatus(
            False, "missing_or_invalid_underlying_timestamp"
        )
    ...
    diagnostics = {
        "option_age_sec": option_age,
        "underlying_age_sec": underlying_age,
        "snapshot_age_sec": snapshot_age,
    }
    if option_age < -EXIT_DECISION_MAX_FUTURE_SKEW_SEC:
        return ExitTruthStatus(False, "future_dated_option_quote", diagnostics)
    if underlying_age < -EXIT_DECISION_MAX_FUTURE_SKEW_SEC:
        return ExitTruthStatus(
            False, "future_dated_underlying_quote", diagnostics
        )
    if snapshot_age < -EXIT_DECISION_MAX_FUTURE_SKEW_SEC:
        return ExitTruthStatus(False, "future_dated_snapshot", diagnostics)
    if option_age > EXIT_DECISION_QUOTE_MAX_AGE_SEC:
        return ExitTruthStatus(False, "stale_option_quote", diagnostics)
    if underlying_age > EXIT_DECISION_QUOTE_MAX_AGE_SEC:
        return ExitTruthStatus(False, "stale_underlying_quote", diagnostics)
    if snapshot_age > EXIT_DECISION_QUOTE_MAX_AGE_SEC:
        return ExitTruthStatus(False, "stale_snapshot", diagnostics)

    cycle_skew = abs((option_ts - underlying_ts).total_seconds())
    option_snapshot_skew = abs((option_ts - snapshot_ts).total_seconds())
    underlying_snapshot_skew = abs(
        (underlying_ts - snapshot_ts).total_seconds()
    )
    diagnostics.update(
        {
            "cycle_skew_sec": cycle_skew,
            "option_snapshot_skew_sec": option_snapshot_skew,
            "underlying_snapshot_skew_sec": underlying_snapshot_skew,
        }
    )
    if max(
        cycle_skew, option_snapshot_skew, underlying_snapshot_skew
    ) > EXIT_DECISION_MAX_CYCLE_SKEW_SEC:
        return ExitTruthStatus(False, "cross_cycle_quote_pair", diagnostics)
    if not str(snap.get("cycle_id") or "").strip():
        return ExitTruthStatus(False, "missing_cycle_id", diagnostics)

    option_provider = str(
        snap.get("option_quote_provider") or ""
    ).lower().strip()
    underlying_provider = str(
        snap.get("underlying_quote_provider") or ""
    ).lower().strip()
    option_domain = str(
        snap.get("option_quote_domain") or ""
    ).lower().strip()
    underlying_domain = str(
        snap.get("underlying_quote_domain") or ""
    ).lower().strip()
    if (
        not option_provider
        or not underlying_provider
        or "unknown" in option_provider
        or "unknown" in underlying_provider
    ):
        return ExitTruthStatus(False, "missing_quote_provider", diagnostics)
    if option_provider != underlying_provider:
        return ExitTruthStatus(
            False, "cross_provider_quote_pair", diagnostics
        )
    if (
        not option_domain
        or not underlying_domain
        or "unproven" in option_domain
        or "unproven" in underlying_domain
    ):
        return ExitTruthStatus(False, "missing_quote_domain", diagnostics)
    if option_domain != underlying_domain:
        return ExitTruthStatus(False, "cross_domain_quote_pair", diagnostics)
    if _positive(snap.get("option_bid")) <= 0:
        return ExitTruthStatus(
            False, "missing_executable_option_bid", diagnostics
        )
    if _positive(snap.get("underlying_midpoint")) <= 0:
        return ExitTruthStatus(False, "missing_underlying_quote", diagnostics)
    return ExitTruthStatus(True, "fresh_coherent_snapshot", diagnostics)
...
def _canonical_reason_code(decision: _Any) -> str:
    ...
    # Soft families are checked before underlying-stop text. A soft reason such
    # as "underlying not confirming" must never become a confirmed stop merely
    # because it contains the words UNDERLYING, STOP, and CONFIRMING.
    if explicit in {
    ...
    if explicit in {"TOUCHED_PROFIT_STOP", "PROFIT_LOCK"} or (
        "TOUCHED PROFIT STOP" in reason
    ):
        return "TOUCHED_PROFIT_FLOOR"
    if explicit == "SMALL_WIN_LOCK" or "SMALL WIN" in reason:
        return "SMALL_WIN_CAPTURE"
    if explicit in {"RUNNER_TRAIL", "TRAILING_STOP"} or any(
        fragment in reason
        for fragment in ("RUNNER TRAIL", "TRAILING STOP")
    ):
        return "RUNNER_TRAIL"
    if explicit in {"HARD_STOP", "MAX_LOSS", "EMERGENCY_STOP"} or (
        "HARD STOP" in reason
    ):
        return "HARD_OPTION_STOP"
    if explicit == "STOP_HIT" or "STOP HIT" in reason:
...
# Public compatibility seam. Existing callers and the one-contract guard must
# continue receiving the original classifier and original reason-code families.
_classify_exit_decision = _base_classify_exit_decision
...
def _deferred(
    pos: ManagedPosition, status: ExitTruthStatus
) -> ExitDecision:
    ...
    return ExitDecision(
        ...
        0,
        (
            "SOFT EXIT DEFERRED — same-cycle underlying truth invalid "
            f"({status.reason}); refresh requested={refreshed}"
        ),
        "NORMAL",
        _decision_pnl(pos),
...
def _grace(
    pos: ManagedPosition, now_utc: _datetime
) -> tuple[bool, float]:
...
def _suppressed(
    ...
    pos.soft_exit_last_diagnostic = {
        ...
        "suppressed_rule": code,
        "confirming_reason": reason,
        "grace_elapsed_sec": elapsed,
    ...
    return ExitDecision(
        ...
        0,
        (
            f"{code} suppressed — fresh underlying confirms thesis "
            f"({reason}); grace={elapsed:.1f}/"
            f"{SOFT_EXIT_THESIS_GRACE_SEC:.1f}s"
        ),
        "NORMAL",
        _decision_pnl(pos),
...
def _soft_candidate(pos: ManagedPosition) -> str:
    ...
    if (
        getattr(pos, "touched_profit", False)
        and int(getattr(pos, "scale_outs_done", 0) or 0) == 0
        and pnl <= _profit_floor(pos)
    ):
        return "TOUCHED_PROFIT_FLOOR"
    peak = float(getattr(pos, "peak_executable_pnl_pct", 0.0) or 0.0)
    if peak >= float(getattr(_base, "IMMEDIATE_TP_PCT", 0.12)):
        if (
            peak - pnl
            >= float(getattr(_base, "TRAIL_DROP_FROM_PEAK", 0.10))
            or pnl <= 0
        ):
            return "RUNNER_TRAIL"
    ...
    if max_profit >= float(getattr(_base, "SMALL_WIN_PCT", 0.12)):
        floor = max(
            0.03,
            max_profit - float(getattr(_base, "SMALL_WIN_TRAIL", 0.08)),
        )
        if 0 < pnl <= floor:
            return "SMALL_WIN_CAPTURE"
    if pnl <= float(_os.getenv("SOFT_LOSS_STOP_PCT", "-0.12")):
        return "SOFT_OPTION_STOP"
    if (
        not getattr(pos, "touched_profit", False)
        and float(_base._position_age_minutes(pos)) >= 20
        and pnl <= -0.08
    ):
        return "SOFT_OPTION_STOP"
    return ""
...
def _eod(
    pos: ManagedPosition, now_et: _datetime
) -> _Optional[ExitDecision]:
    hour, minute = now_et.hour, now_et.minute
    past = hour > _base.EOD_HARD_CLOSE_HOUR or (
        hour == _base.EOD_HARD_CLOSE_HOUR
        and minute >= _base.EOD_HARD_CLOSE_MIN
    )
    if (
        (past or hour >= 16)
        ...
        return ExitDecision(
            "CLOSE_ALL",
            int(getattr(pos, "quantity_remaining", 0) or 0),
            f"EOD FORCE CLOSE -- {hour}:{minute:02d} ET",
            "IMMEDIATE",
            _decision_pnl(pos),
...
def evaluate_exit(
    pos: ManagedPosition, now_et: _Optional[_datetime] = None
):
    if not _enabled(pos):
    ...
    now_utc = (
        now_et.astimezone(_timezone.utc)
        if now_et.tzinfo
        else now_et.replace(tzinfo=_base.ET).astimezone(_timezone.utc)
    )
    ...
    if pnl <= hard_stop:
        return ExitDecision(
            ...
            int(getattr(pos, "quantity_remaining", 0) or 0),
            (
                "HARD STOP -- executable bid P&L "
                f"{pnl*100:.1f}% exceeded {hard_stop*100:.1f}% max loss"
            ),
            "IMMEDIATE",
            pnl,
    ...
    if candidate:
        ...
        if confirming and not pos.is_at_stop:
            in_grace, elapsed = _grace(pos, now_utc)
            if in_grace:
                return _suppressed(
                    pos, candidate, confirming_reason, elapsed
                )
...
def _runtime_evaluate_exit(pos, now_et=None):
    """Let later public wrappers, including the existing one-contract guard,
    remain authoritative while the private legacy loop calls this proxy."""
    current = globals().get("evaluate_exit", evaluate_exit)
    return current(pos, now_et)


def build_exit_decision_stamp(
    pos: ManagedPosition, decision: _Any, **kwargs
) -> dict:
    ...
    payload.update(
        {
            ...
            "executable_option_bid": _positive(snap.get("option_bid"))
            or _positive(getattr(pos, "current_bid", 0.0)),
            "analytics_option_midpoint": _positive(
                snap.get("option_midpoint")
            )
            or _positive(getattr(pos, "analytics_mark_price", 0.0)),
            "underlying_bid": _positive(snap.get("underlying_bid")),
            "underlying_ask": _positive(snap.get("underlying_ask")),
            "underlying_midpoint": _positive(
                snap.get("underlying_midpoint")
            ),
            "option_quote_timestamp": snap.get("option_quote_timestamp"),
            "underlying_quote_timestamp": snap.get(
                "underlying_quote_timestamp"
            ),
            "option_quote_provider": snap.get("option_quote_provider"),
            "underlying_quote_provider": snap.get(
                "underlying_quote_provider"
            ),
            "option_quote_domain": snap.get("option_quote_domain"),
            "underlying_quote_domain": snap.get(
                "underlying_quote_domain"
            ),
            "profit_lock_armed_at": (
                armed_at.isoformat()
                if isinstance(armed_at, _datetime)
                else armed_at
            ),
            "profit_lock_arm_bid": float(
                getattr(pos, "profit_lock_arm_bid", 0.0) or 0.0
            ),
            "profit_lock_arm_pnl_pct": float(
                getattr(pos, "profit_lock_arm_pnl_pct", 0.0) or 0.0
            ),
            "peak_executable_bid": float(
                getattr(pos, "peak_executable_bid", 0.0) or 0.0
            ),
            "peak_executable_pnl_pct": float(
                getattr(pos, "peak_executable_pnl_pct", 0.0) or 0.0
            ),
            "profit_lock_confirmation_count": int(
                getattr(pos, "profit_lock_confirmation_count", 0) or 0
            ),
            "decision_bound_cycle_id": getattr(
                decision, "_exit_truth_cycle_id", ""
            ),
        }
    )
    return payload
...
def _hydrate(pos: _Any, row: dict) -> None:
    if not _enabled(pos):
        return
    ...
    if isinstance(meta, str):
        try:
            ...
            meta = {}
    truth = (
        dict((meta or {}).get("soft_exit_truth") or {})
        if isinstance(meta, dict)
        else {}
    )
    for key in (
        "profit_lock_arm_bid",
        "profit_lock_arm_pnl_pct",
        "peak_executable_bid",
        "peak_executable_pnl_pct",
    ):
        try:
            setattr(pos, key, float(truth.get(key) or 0.0))
        except Exception:
            pass
    pos.profit_lock_confirmation_count = int(
        truth.get("profit_lock_confirmation_count") or 0
    )
    pos.profit_lock_last_confirmation_cycle_id = str(
        truth.get("profit_lock_last_confirmation_cycle_id") or ""
    )
    for key in (
        "profit_lock_armed_at",
        "profit_lock_confirmation_first_ts",
        "profit_lock_last_confirmation_ts",
        "soft_exit_grace_started_at",
        "soft_exit_last_suppressed_at",
        "soft_exit_last_deferred_at",
    ):
        setattr(pos, key, _coerce_dt(truth.get(key)))
    if isinstance(truth.get("profit_lock_quote_timestamps"), list):
        pos.profit_lock_quote_timestamps = list(
            truth["profit_lock_quote_timestamps"]
        )[-8:]
    if isinstance(truth.get("profit_lock_quote_sources"), list):
        pos.profit_lock_quote_sources = list(
            truth["profit_lock_quote_sources"]
        )[-8:]
    if isinstance(truth.get("decision_quote_snapshot"), dict):
        pos.exit_decision_quote_snapshot = dict(
            truth["decision_quote_snapshot"]
        )
    if isinstance(truth.get("soft_exit_last_diagnostic"), dict):
        pos.soft_exit_last_diagnostic = dict(
            truth["soft_exit_last_diagnostic"]
        )
    peak = float(getattr(pos, "peak_executable_pnl_pct", 0.0) or 0.0)
    if peak > 0:
        pos.peak_pnl_pct = peak
        pos.max_profit_seen = peak
    if getattr(pos, "profit_lock_armed_at", None) is not None:
...
class APExitEngine(_BaseAPExitEngine):
    ...
    def add_position(self, pos: ManagedPosition):
        try:
            ...
            pos._soft_exit_refresh_callback = (
                lambda p=pos: self._request_immediate_quote_retry(p)
            )
        except Exception:
            pass
    ...
    def _managed_position_from_row(
        ...
        *,
        ...
        pos = _base_managed_position_from_row(
            ...
            row,
            qty_override,
            prefer_qty_override=prefer_qty_override,
        )
        try:
            pos.executable_bid_soft_exit_truth_enabled = True
            pos._soft_exit_refresh_callback = (
    ...
    def _qpm_enriched_snapshot(
        self, raw: dict, context: dict
    ) -> ExitDecisionQuoteSnapshot:
        position_id = str(
            raw.get("position_id") or raw.get("positionid") or ""
        )
        option_symbol = str(
            raw.get("option_symbol") or raw.get("optionsymbol") or ""
        ).upper()
        ticker = str(raw.get("ticker") or "").upper()
        option_raw = dict(
            (context.get("option_quotes") or {}).get(option_symbol) or {}
        )
        underlying_raw = dict(
            (context.get("underlying_quotes") or {}).get(ticker) or {}
        )
        fallback_ts = _coerce_dt(context.get("snapshot_timestamp")) or _utc_now()
        option_ts = _coerce_dt(
            (context.get("option_quote_timestamps") or {}).get(option_symbol)
        ) or fallback_ts
        underlying_ts = _coerce_dt(
            (context.get("underlying_quote_timestamps") or {}).get(ticker)
        ) or fallback_ts
        snapshot_ts = _coerce_dt(context.get("snapshot_timestamp")) or max(
            option_ts, underlying_ts
        )

        option_bid = _positive(option_raw.get("bid"))
        option_ask = _positive(option_raw.get("ask"))
        option_mid = (
            round((option_bid + option_ask) / 2.0, 4)
            if option_bid > 0 and option_ask > 0
            else _positive(option_raw.get("mark"))
            or _positive(option_raw.get("last"))
        )
        underlying_bid = _positive(underlying_raw.get("bid"))
        underlying_ask = _positive(underlying_raw.get("ask"))
        underlying_last = (
            _positive(underlying_raw.get("last"))
            or _positive(underlying_raw.get("price"))
            or _positive(underlying_raw.get("mark"))
            or _positive(underlying_raw.get("close"))
        )
        underlying_mid = (
            round((underlying_bid + underlying_ask) / 2.0, 4)
            if underlying_bid > 0 and underlying_ask > 0
            else underlying_last
        )

        modes = context.get("original_modes") or {}
        modes_by_symbol = context.get("original_modes_by_symbol") or {}
        execution_mode = _mode(
            modes.get(position_id)
            or modes_by_symbol.get(option_symbol)
            or context.get("default_execution_mode")
        )
        provider = str(
            context.get("quote_provider") or "unknown"
        ).lower().strip()
        domain = str(
            context.get("quote_domain") or "unknown"
        ).lower().strip()
        option_provider = str(
            option_raw.get("provider") or provider
        ).lower().strip()
        underlying_provider = str(
            underlying_raw.get("provider") or provider
        ).lower().strip()
        option_domain = str(
            option_raw.get("data_domain")
            or option_raw.get("domain")
            or domain
        ).lower().strip()
        underlying_domain = str(
            underlying_raw.get("data_domain")
            or underlying_raw.get("domain")
            or domain
        ).lower().strip()
        ...
        if pos is None:
            pos = next(
                (
                    candidate
                    for candidate in self.active_positions()
                    if str(
                        getattr(candidate, "option_symbol", "") or ""
                    ).upper()
                    == option_symbol
                ),
                None,
    ...
    def apply_exit_decision_snapshots(self, snapshots: list[dict]) -> None:
        ...
        by_id = {
            str(getattr(pos, "position_id", "") or ""): pos
            for pos in active
        }
        by_symbol = {
            str(getattr(pos, "option_symbol", "") or "").upper(): pos
            for pos in active
        }
        for raw in list(snapshots or []):
            snap = self._qpm_enriched_snapshot(dict(raw or {}), context)
            pos = by_id.get(snap.position_id) or by_symbol.get(
                snap.option_symbol
            )
            if pos is None:
                continue
            try:
                ...
                continue
            if (
                snap.execution_mode not in {"paper", "live"}
                or snap.execution_mode != _mode(pos.execution_mode)
            ):
            ...
            _apply_option_quote_for_decision(
                ...
                bid=snap.option_bid,
                ask=snap.option_ask,
                mark=snap.option_midpoint,
                quote_ts=snap.option_quote_timestamp,
                source=(
                    f"{snap.option_quote_provider}:"
                    f"{snap.option_quote_domain}:executable_bid"
                ),
            )
            if snap.underlying_midpoint > 0:
                pos.current_underlying = snap.underlying_midpoint
                pos.last_underlying_quote_update_ts = (
                    snap.underlying_quote_timestamp
                )
            ...
            pos.exit_decision_quote_snapshot = snap.to_dict()
            truth = _validate_exit_truth_snapshot(
                pos, now_utc=snap.snapshot_timestamp
            )

            if snap.option_bid > 0 and float(pos.entry_price or 0.0) > 0:
                pnl = (snap.option_bid - float(pos.entry_price)) / float(
                    pos.entry_price
                )
                ...
                pos.peak_pnl_pct = max(
                    0.0, float(pos.peak_executable_pnl_pct or 0.0)
                )
                pos.max_profit_seen = max(
                    0.0, float(pos.peak_executable_pnl_pct or 0.0)
                )
                ...
                if pos.profit_lock_armed_at is None:
                    ...
                    consecutive = bool(
                        ...
                        and (
                            snap.option_quote_timestamp - prior_ts
                        ).total_seconds()
                    ...
                    if truth.valid and pnl >= TOUCHED_PROFIT_ARM_PCT:
                        if not consecutive:
                            ...
                            pos.profit_lock_confirmation_first_ts = (
                                snap.option_quote_timestamp
                            )
                            pos.profit_lock_quote_timestamps = []
                            pos.profit_lock_quote_sources = []
                        else:
                            pos.profit_lock_confirmation_count += 1
                        pos.profit_lock_last_confirmation_ts = (
                            snap.option_quote_timestamp
                        )
                        pos.profit_lock_last_confirmation_cycle_id = (
                            snap.cycle_id
                        ...
                        pos.profit_lock_quote_timestamps = (
                            list(pos.profit_lock_quote_timestamps or [])
                            + [snap.option_quote_timestamp.isoformat()]
                        )[-8:]
                        pos.profit_lock_quote_sources = (
                            list(pos.profit_lock_quote_sources or [])
                            + [
                                f"{snap.option_quote_provider}:"
                                f"{snap.option_quote_domain}:bid"
                            ]
                        )[-8:]
                        if (
                            pos.profit_lock_confirmation_count
                            >= TOUCHED_PROFIT_CONFIRM_SNAPSHOTS
                        ):
                            pos.profit_lock_armed_at = (
                                snap.option_quote_timestamp
                            )
    ...
    def _meta_payload(self, pos: ManagedPosition) -> dict:
        ...
        return {
            ...
            "profit_lock_arm_pnl_pct": float(
                pos.profit_lock_arm_pnl_pct or 0.0
            ),
            "peak_executable_bid": float(pos.peak_executable_bid or 0.0),
            "peak_executable_pnl_pct": float(
                pos.peak_executable_pnl_pct or 0.0
            ),
            "profit_lock_confirmation_count": int(
                pos.profit_lock_confirmation_count or 0
            ),
            "profit_lock_confirmation_first_ts": iso(
                pos.profit_lock_confirmation_first_ts
            ),
            "profit_lock_last_confirmation_ts": iso(
                pos.profit_lock_last_confirmation_ts
            ),
            "profit_lock_last_confirmation_cycle_id": str(
                pos.profit_lock_last_confirmation_cycle_id or ""
            ),
            "profit_lock_quote_timestamps": list(
                pos.profit_lock_quote_timestamps or []
            )[-8:],
            "profit_lock_quote_sources": list(
                pos.profit_lock_quote_sources or []
            )[-8:],
            "soft_exit_grace_started_at": iso(
                pos.soft_exit_grace_started_at
            ),
            "soft_exit_last_suppressed_at": iso(
                pos.soft_exit_last_suppressed_at
            ),
            "soft_exit_last_deferred_at": iso(
                pos.soft_exit_last_deferred_at
            ),
            "soft_exit_last_diagnostic": dict(
                pos.soft_exit_last_diagnostic or {}
            ),
    ...
    def _persist_soft_exit_truth_to_db(
        self, pos: ManagedPosition, *, force: bool = False
    ) -> bool:
        if not _enabled(pos):
            return False
        ...
        transition_signature = _json.dumps(
            {
                ...
                "confirmation_count": payload.get(
                    "profit_lock_confirmation_count"
                ),
        ...
        last_epoch = float(
            getattr(pos, "_soft_exit_truth_last_persist_epoch", 0.0)
            or 0.0
        )
        transition = transition_signature != str(
            getattr(pos, "_soft_exit_truth_last_transition_signature", "")
            or ""
        )
        ...
        try:
            ...
            def update():
                with conn() as cursor:
                    cursor.execute(
                        ...
                        (
                            patch,
                            float(pos.peak_executable_pnl_pct or 0.0),
                            float(pos.peak_executable_pnl_pct or 0.0),
                            bool(pos.touched_profit),
                            float(pos.current_bid or 0.0),
                            _decision_pnl(pos),
            ...
            if rowcount == 1:
                ...
                pos._soft_exit_truth_last_transition_signature = (
                    transition_signature
                )
                return True
            log.warning(
                "SOFT_EXIT_TRUTH_PERSIST_ROWCOUNT client=%s pos=%s "
                "mode=%s contract=%s rowcount=%s",
                identity.client_id,
                identity.position_id,
                identity.execution_mode,
                identity.contract,
            ...
            log.debug(
                "soft-exit truth persistence failed non-fatally: %s", exc
            )
        return False
    ...
    def adopt_canonical_position_identity(self, *args, **kwargs):
        contract = str(
            kwargs.get("contract") or (args[0] if args else "") or ""
        ).upper()
        protected_repairs = [
            pos
            for pos in list(getattr(self, "_positions", []) or [])
            if _enabled(pos)
            and str(getattr(pos, "option_symbol", "") or "").upper()
            == contract
            and str(getattr(pos, "position_id", "") or "").startswith(
                "broker-repair-"
            )
        ...
        state = [
            {
                "armed_at": getattr(pos, "profit_lock_armed_at", None),
                "arm_bid": getattr(pos, "profit_lock_arm_bid", 0.0),
                "arm_pnl": getattr(pos, "profit_lock_arm_pnl_pct", 0.0),
                "peak_bid": getattr(pos, "peak_executable_bid", 0.0),
                "peak_pnl": getattr(
                    pos, "peak_executable_pnl_pct", 0.0
                ),
            }
            for pos in protected_repairs
        ]
        result = _base_adopt_canonical_position_identity(
            self, *args, **kwargs
        )
        if not getattr(result, "adopted", False) or not state:
            return result

        canonical_id = str(
            kwargs.get("canonical_position_id")
            or (args[1] if len(args) > 1 else "")
            or ""
        )
        canonical = getattr(self, "_positions_by_id", {}).get(canonical_id)
        if canonical is None or not _enabled(canonical):
            return result
        best = max(
            state, key=lambda item: float(item.get("peak_pnl") or 0.0)
        )
        current_peak = float(
            getattr(canonical, "peak_executable_pnl_pct", 0.0) or 0.0
        )
        if float(best.get("peak_pnl") or 0.0) > current_peak:
            canonical.peak_executable_pnl_pct = float(best["peak_pnl"])
            canonical.peak_executable_bid = float(
                best.get("peak_bid") or 0.0
            )
        ...
        if armed_at is not None:
            ...
            canonical.profit_lock_arm_bid = float(
                best.get("arm_bid") or 0.0
            )
            canonical.profit_lock_arm_pnl_pct = float(
                best.get("arm_pnl") or 0.0
            )
    ...
    @staticmethod
    def _sync_legacy_submit_seams() -> None:
        # The shadowed legacy method resolves globals in its private module.
        # Synchronize only the public monkeypatch seams used by existing guards
        # and tests. This does not change production behavior when unpatched.
        _base._classify_exit_decision = globals().get(
            "_classify_exit_decision", _base_classify_exit_decision
        )
        _base._is_option_quote_stale = globals().get(
            "_is_option_quote_stale", _base_is_option_quote_stale
        )
    ...
    def _submit_exit_decision(
        ...
        *,
        ...
        self._sync_legacy_submit_seams()
        if _enabled(pos):
            canonical = _canonical_reason_code(decision)
            decision.reason_code = canonical
            if canonical in _CONTEXT_CODES:
                truth = _validate_exit_truth_snapshot(pos)
                current_cycle = str(_snapshot(pos).get("cycle_id") or "")
                decision_cycle = str(
                    getattr(decision, "_exit_truth_cycle_id", "") or ""
                )
                if (
                    not truth.valid
                    or not decision_cycle
                    or decision_cycle != current_cycle
                ):
                    reason = (
                        truth.reason
                        if not truth.valid
                        else "decision_snapshot_cycle_changed"
                    )
                    deferred = _deferred(
                        pos,
                        ExitTruthStatus(
                    ...
                    return False
                decision.suggested_limit = float(
                    getattr(pos, "current_bid", 0.0) or 0.0
                )
...
APExitEngine.__module__ = __name__

# Private legacy loop wiring. Only the evaluator and quote writer are proxied;
# the legacy classifier remains unchanged for every unprotected position.
_base.ManagedPosition = ManagedPosition
_base.APExitEngine = APExitEngine
_base.evaluate_exit = _runtime_evaluate_exit
...
globals().update(
    {
        ...
        "_canonical_reason_code": _canonical_reason_code,
