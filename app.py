...
def admin_force_exit_position(position_id: str):
    ...
    try:
        ...
        # pos._proof_staged so _finalize_proof can write proof_trades when
        # the fill confirms. Admin force exit calls _submit_exit_decision
        # directly, skipping that callback chain entirely. Result: the fill
        # lands, mark_position_closed fires, _finalize_proof sees staged=None,
        # returns immediately, and the trade never appears in proof_trades.
        #
        # Fix: stage proof here immediately after submit, using current position
        # state. We then write proof_trades directly (best-effort) since we
        # won't get a broker fill callback through the normal path reliably.
        # The exit_fill_price will be the current bid (best estimate available
        # at force-exit time; the actual fill may differ slightly for paper).
        try:
