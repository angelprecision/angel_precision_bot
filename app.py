...
def create_app() -> Flask:
    ...
    def admin_morning_handoff_audit_post():
        ...
        if not dry_run and body.get("use_run_lock", True):
            ...
            if not bool((run_lock or {}).get("acquired")):
                return jsonify({
                    "ok": True,
            ...
            run_lock_owner_token = str((run_lock or {}).get("owner_token") or "")
        ...
        blocked_clients = [
            client_id
            for client_id, row in readiness_results.items()
            if isinstance(row, dict)
            and str((row or {}).get("status") or "").upper() == "BLOCKED"
        ]
        readiness_not_ok_clients = [
            client_id
            for client_id, row in readiness_results.items()
        ...
        handoff_not_ok_clients = [
            client_id
            for client_id, row in handoff_results.items()
            if isinstance(row, dict) and row.get("ok") is False
        ]
        no_clients_audited = len(handoff_results) == 0
        ok = not any((
            errors,
            explicit_target and no_clients_audited,
            blocked_clients,
            readiness_not_ok_clients,
            no_clients_audited,
            handoff_not_ok_clients,
        ...
        if (
            ...
            summary = {
                ...
                "readiness_not_ok_clients": readiness_not_ok_clients,
                "handoff_not_ok_clients": handoff_not_ok_clients,
                "explicit_target_selected_zero_runners": explicit_target and no_clients_audited,
            }
            if ok:
                mark_run_lock_completed(run_lock_key, run_lock_owner_token, summary)
            else:
                if errors:
                    fail_reason = f"{len(errors)} client error(s)"
                elif explicit_target and no_clients_audited:
                    fail_reason = "requested_clients_not_in_active_runners_for_mode"
                elif blocked_clients:
                    fail_reason = "readiness_blocked"
                elif readiness_not_ok_clients:
                    fail_reason = "readiness_not_ok"
                elif no_clients_audited:
                    fail_reason = "no_clients_audited"
                else:
                    fail_reason = "handoff_result_not_ok"
                mark_run_lock_failed(run_lock_key, run_lock_owner_token, fail_reason, summary)
        ...
        if explicit_target and len(handoff_results) == 0:
            ...
            response_body["requested_mode"] = mode
