...
CORS(app, resources={
    r"/*": {
        "origins": [
            ...
            "https://angelprecision.vercel.app",
            "https://angel-precision.vercel.app",
            "http://localhost:3000",
            "http://localhost:5173",
            "http://localhost:8080",
        ],
...
def admin_activate_subscription():
    ...
    try:
        ...
        # Full automated onboarding: DB provision + welcome email + Discord
        from ap.onboarding import run_onboarding
        onboarding = run_onboarding(
            email=email,
            name=member.get("name") or email,
            access_code=access_code,
            tier=tier,
            equity=float(member.get("starting_balance") or 25000),
        )

        return jsonify({
            "ok":             True,
            "message":        f"Activated for {email}",
            "member":         member,
            "db_provisioned": onboarding["db_provisioned"],
            "email_sent":     onboarding["email_sent"],
            "discord_posted": onboarding["discord_posted"],
            "onboarding_errors": onboarding["errors"],
        })
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
def admin_deactivate_subscription():
    data = request.get_json() or {}
    email  = data.get("email")
    reason = data.get("reason", "subscription ended")
    if not email:
    ...
    try:
        ...
        member = result.data[0]
        from ap.onboarding import post_discord_deactivation
        post_discord_deactivation(
            name=member.get("name") or email,
            email=email,
            reason=reason,
        )
...
@auth_required
def send_signal_proxy():
    """
    Proxy a manual signal from the dashboard to the bot.
    Used by the manual signal sender panel in the Account tab.
    Forwards to the bot's /signal endpoint with proper HMAC auth.
    """
    import hmac as hmac_lib, hashlib, time, json as json_lib, os
    ...
    bot_url = os.getenv("BOT_SIGNAL_URL",
        "https://angel-precision-bot-official-1.onrender.com/signal")
    signing_secret = os.getenv("SIGNING_SECRET", "").encode()
    client_id      = os.getenv("BOT_CLIENT_ID", "default_v4")

    body = json_lib.dumps(data).encode("utf-8")
    ts   = str(int(time.time()))
    msg  = f"{ts}.".encode() + body
    sig  = hmac_lib.new(signing_secret, msg, hashlib.sha256).hexdigest() if signing_secret else ""

    try:
        # Render free tier sleeps after inactivity -- wake it up first
        # then send the actual signal
        try:
            req_lib.get(bot_url.replace("/signal", "/health"), timeout=30)
        except Exception:
            pass  # wake attempt -- ignore errors
        ...
        r = req_lib.post(
            ...
            timeout=35,   # generous timeout after wake
        )
        return jsonify({"ok": r.status_code < 300, "status": r.status_code,
                        "response": r.json() if r.content else {}}), 200
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
