...
-- =============================================================================
-- Records what the exit system saw and did at every critical step:
-- quote -> decision -> submit -> ack/reject -> fill -> close.
--
...
CREATE INDEX IF NOT EXISTS idx_exit_ledger_contract_created
    ON exit_decision_ledger (contract, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_exit_ledger_payload_gin
    ON exit_decision_ledger USING GIN (payload);

COMMENT ON TABLE exit_decision_ledger IS
'Best-effort exit audit trail: records quote state, P&L, decision, submit, broker, fill, and error context for every protected exit path.';
