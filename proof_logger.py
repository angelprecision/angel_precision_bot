...
class FunnelCounter:
    ...
    def __init__(self):
        ...
        self._day      = None   # current ET date — auto-resets on new day
        self.reset()
...
class APProofLogger:
    ...
    Usage:
        ...
        # When a position OPENS (FIX 2):
        proof.log_position_opened(pos)
    ...
    # ── FIX 2: Increment executed at OPEN ─────────────────────────────────────
    ...
    # ── FIX 3+4+7: Daily summary rebuilt from Supabase ───────────────────────
    ...
    def generate_daily_summary(self, trade_date: Optional[date] = None) -> dict:
        ...
        """
        # FIX 4: ET date — not UTC
        today    = trade_date or datetime.now(ET).date()
        snapshot = funnel.snapshot()

        # FIX 3+7: Fetch from Supabase for this ET trading day
        ...
        summary = {
            ...
            # Funnel (from in-memory counter — incremented by gates throughout day)
            ...
            "trades_executed":       snapshot["trades_executed"],   # from OPEN events
        ...
        # FIX 1: Upsert on date,client_email,system_version
        if self.sb:
            try:
    ...
    def _fetch_trades_for_date(self, trade_date: date) -> list[dict]:
        ...
        FIX 4: Uses ET date conversion — not UTC string slicing.
        """
        if not self.sb:
            # Fall back to cache if no Supabase
            with self._lock:
                return [t for t in self._trades_cache
                        if _to_et_date(t.get("closed_at", "")) == trade_date]

        try:
            # Query a 26-hour UTC window covering the ET trading day
            # (ET is UTC-4 in summer, UTC-5 in winter — 26h covers both safely)
    ...
    # ── FIX 5+6: 10-day proof — true 10 trading days + full stats ─────────────

    def generate_10day_proof(self) -> dict:
        """
        Generate the client-facing 10-day proof summary.
        FIX 5: Filters to last 10 distinct ET trading days (not all-time).
        FIX 6: Includes median, avg hold time, worst trade, exit reason breakdown.
        """
        if not self.sb:
            return {"status": "no_supabase"}

        try:
            # Get last 10 distinct trading days from proof_daily_summary
            ...
            # Fetch trades for those days only
            # Convert ET dates to UTC window
        ...
        # FIX 6: Avg hold time in minutes (using DB hold_minutes if available, else compute)
        hold_times = []
        for t in trades:
        ...
        # Max equity drawdown (cumulative curve)
