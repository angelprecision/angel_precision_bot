...
def test_daily_call_fresh_high_allowed_before_submit_even_without_legacy_confirmation():
    ...
    assert result.metadata["daily_continuation_allowed"] is True
    diag = result.metadata["daily_continuation_diagnostics"]
    assert diag["fresh_intraday_high"] is True
    assert diag["client_id"] == "jasoncosby1@gmail.com"
    assert diag["execution_mode"] == "live"
    assert diag["adapter"] == "entry_confirmation_pre_submit"
...
def test_daily_call_touch_then_fade_blocks_before_submit():
    ...
    assert result.metadata["daily_continuation_allowed"] is False
    assert result.metadata["daily_continuation_diagnostics"]["price_back_through_trigger"] is True
...
def test_daily_put_fresh_low_allowed_before_submit():
    ...
    assert result.passed is True
    assert result.metadata["daily_continuation_allowed"] is True
    assert result.metadata["daily_continuation_diagnostics"]["fresh_intraday_low"] is True


def test_daily_missing_context_blocks_before_legacy_fast_path(monkeypatch):
    monkeypatch.setattr(entry_confirmation, "_fetch_daily_continuation_candles", lambda ticker: [])
...
def test_daily_opening_breach_no_extension_blocks():
    ...
    assert result.metadata["daily_continuation_diagnostics"]["opening_breach"] is True
...
def test_daily_continuation_can_be_disabled_for_emergency_rollback(monkeypatch):
    ...
    assert result.passed is True
    assert result.metadata["daily_continuation_allowed"] is True
    assert result.metadata["daily_continuation_reason"] == "daily_continuation_skipped:disabled"
