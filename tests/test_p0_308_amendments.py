...
class TestBrokerFlatProductionShape:
    ...
    def test_unrelated_negative_stock_quantity_does_not_invalidate_flat_snapshot(self):
        """A legitimate unrelated short stock row must not erase target flatness."""
        result = self._run_broker_truth(
            list_positions_return=[{"symbol": "AAPL", "quantity": -100}]
        )
    ...
    def test_unrelated_negative_short_option_does_not_invalidate_flat_snapshot(self):
        """A legitimate unrelated short option row must be ignored for this target."""
        result = self._run_broker_truth(
            list_positions_return=[
                {
    ...
    def test_target_contradictory_or_malformed_quantity_is_unknown_not_flat(self, row):
        """The target row remains fail-closed for negative or malformed quantity."""
        result = self._run_broker_truth(list_positions_return=[row])
    ...
    def test_malformed_unrelated_identity_keeps_snapshot_unknown(self):
        """Conflicting unrelated identities make the complete snapshot untrustworthy."""
        result = self._run_broker_truth(
            list_positions_return=[
                {
