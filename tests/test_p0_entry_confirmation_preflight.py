...
os.environ.update({
    "ENTRY_CONFIRMATION_REQUIRED_DEFAULT": "1",
...
def test_explicit_no_confirmation_required_passes():
    r = _call(plan=_plan_with_gate(confirmation_required=False))
    assert r.passed is True
    assert r.metadata.get("confirmation_required") is False
    assert r.metadata.get("confirmation_required_source") == "hybrid_client_quality_gate"


def test_missing_gate_metadata_defaults_confirmation_required():
    r = _call(plan=_plan_missing_gate())
    assert r.passed is True
    assert r.metadata.get("confirmation_required") is True
    assert r.metadata.get("confirmation_required_source") == "global_default"


def test_missing_gate_metadata_no_longer_skips_stale_quote(monkeypatch):
    monkeypatch.setenv("ENTRY_CONFIRMATION_REQUIRED_DEFAULT", "1")
    r = _call(plan=_plan_missing_gate(), live_quote_age_ms=15_000)
    assert r.passed is False
    assert r.fail_reason == "entry_confirm_failed_stale_quote"
    assert r.metadata.get("confirmation_required") is True


def test_live_missing_gate_uses_live_default(monkeypatch):
    monkeypatch.delenv("ENTRY_CONFIRMATION_REQUIRED_DEFAULT", raising=False)
    monkeypatch.setenv("ENTRY_CONFIRMATION_REQUIRED_LIVE_DEFAULT", "1")
    p = _plan_missing_gate()
    p.execution_mode = "live"
    r = _call(plan=p, live_quote_age_ms=15_000)
    assert r.passed is False
    assert r.fail_reason == "entry_confirm_failed_stale_quote"
    assert r.metadata.get("execution_mode") == "LIVE"
    assert r.metadata.get("confirmation_required_source") == "live_env_default"


def test_invalid_direction_fails_closed_even_when_confirmation_explicitly_disabled():
    r = _call(plan=_plan_with_gate(confirmation_required=False), direction="SIDEWAYS")
    assert r.passed is False
    assert r.fail_reason == "entry_confirm_failed_invalid_direction"
...
def test_missing_quote_age_blocks():
    r = _call(live_quote_age_ms=None)
    assert r.passed is False
    assert r.fail_reason == "entry_confirm_failed_missing_quote_age"


def test_stale_quote_blocks():
    r = _call(live_quote_age_ms=15_000)
    assert r.passed is False
    assert r.fail_reason == "entry_confirm_failed_stale_quote"
...
def test_no_quote_blocks():
    ...
    assert r.passed is False
    assert r.fail_reason == "entry_confirm_failed_quote_missing_bid_ask"


def test_one_sided_quote_blocks():
    r = _call(live_bid=None, live_ask=3.60)
    assert r.passed is False
    assert r.fail_reason == "entry_confirm_failed_quote_missing_bid_ask"


def test_zero_bid_or_ask_blocks():
    r = _call(live_bid=0.0, live_ask=3.60)
    assert r.passed is False
    assert r.fail_reason == "entry_confirm_failed_zero_bid_ask"


def test_inverted_quote_blocks():
    r = _call(live_bid=3.80, live_ask=3.60)
    assert r.passed is False
    assert r.fail_reason == "entry_confirm_failed_inverted_quote"


def test_fresh_quote_passes():
    r = _call(live_quote_age_ms=3000)
    assert r.passed is True
...
def test_passed_metadata_fields():
    ...
    required = [
        ...
        "execution_mode", "confirmation_required_source",
    ]
...
def test_paper_quote_lag_warning():
    r = _call(sandbox_mode=True, live_bid=3.30, live_ask=3.40, decision_option_price=3.50)
    assert r.passed is True
