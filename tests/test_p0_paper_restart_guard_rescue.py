...
def test_watcher_owned_watching_row_is_restored_to_watching():
    plan = build_paper_restart_guard_rescue_plan(
        queue_rows=[_queue_row(queue_id=21)],
...
def test_manual_rescue_route_restores_queue_only_rows_to_watching_with_overnight_flags():
    src = (REPO_ROOT / "app.py").read_text()
    assert "status = 'WATCHING'" in src
    assert "last_error = 'after_hours_deferred:awaiting_overnight_reeval'" in src
    assert "'force_overnight_reeval_only', true" in src
    assert "'do_not_queue_directly', true" in src
    assert "'manual_rescue_route', 'overnight_reeval_only'" in src
