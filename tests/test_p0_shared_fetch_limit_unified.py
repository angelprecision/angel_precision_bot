...
def test_ap_signals_fetch_uses_the_same_env_controlled_limit_variable():
    """Both queries reference _fetch_limit."""
    # The Supabase call must use .limit(_fetch_limit).
    assert ".limit(_fetch_limit)" in OV_SRC
...
def test_default_fetch_limit_still_accommodates_up_to_500_rows(monkeypatch):
    ...
    assert seen_limits == [500], (
        f"expected default Supabase limit of 500; got {seen_limits}"
    )
